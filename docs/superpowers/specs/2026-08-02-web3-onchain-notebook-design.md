...
本次在外置 Chrome 的 Remix 2.5.3、Remix VM（Osaka）观察到：`SimpleBank`（短地址 `0xD4F...2cbee`）存入 1 ETH 后读取 `1000000000000000000` wei，取出 0.4 ETH 后内部余额与合约余额均为 0.6 ETH；`RedPacket`（短地址 `0x332...D4B6D`）以 `c=2`、`_isEqual=true` 和 2 ETH 部署，Account 2 与 Account 3 各领取 1 ETH，最终 `count=0`、合约余额为 0 ETH。成功后 3 次重复领取均以 `count must > 0` 回滚且状态不变。该自动化交互曾出现异常，记录保留该事实；这些临时 VM 地址不是 Sepolia 证据。`refund()` 的 24 小时到期分支未等待实时时间验证，仍为未覆盖。
