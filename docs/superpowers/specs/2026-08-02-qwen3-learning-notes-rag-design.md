...
> 状态：已被 `2026-08-02-yideng-agent-rag-training-design.md` 的综合设计取代。本文件保留为历史设计依据；发生冲突时以综合设计为准，现有 50 条 train 仅为 smoke，不代表正式训练完成。
...
先准备 50 至 100 条冒烟样本跑通训练闭环。正式第一版必须包含至少 300 条高质量 `train`，独立 `validation` 和冻结 `test` 不计入 300；质量允许时推荐约 1000 条。数据按主题和来源分组后划分，禁止近似样本跨集合泄漏。
...
当前只确认 UU 中存在 4070Ti/5070 云真机候选、界面费率为 450 U币/小时且最近只读余额为 1900 U币，实例尚未启动。候选名称不能证明实际 GPU 或显存；启动前必须再次确认费用，启动后先核验 GPU、显存、驱动、CUDA、磁盘和已有模型，再决定训练参数。
