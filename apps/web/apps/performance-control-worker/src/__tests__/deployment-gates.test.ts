...
describe("performance control deployment gates", () => {
	it("keeps deployed migrations immutable and adds a TOTP throttle migration", () => {
		const original = readFileSync(
			resolve(root, "migrations/0001_performance_control.sql"),
			"utf8",
		);
		expect(original).not.toContain("cleanup_required");
		expect(original).not.toContain("control_nonces");
		const upgrade = readFileSync(
			resolve(root, "migrations/0002_mfa_control_security.sql"),
			"utf8",
		);
		...
		const totp = readFileSync(
			resolve(root, "migrations/0003_totp_auth.sql"),
			"utf8",
		);
		expect(totp).toContain("totp_attempts");
		expect(totp).toContain("locked_until");
		const readiness = readFileSync(
			resolve(root, "migrations/0004_dispatch_readiness.sql"),
			"utf8",
		);
		expect(readiness).toContain("probe_token TEXT NOT NULL");
	});

	it("migrates a digest-only legacy snapshot as unavailable audit evidence", () => {
		const directory = mkdtempSync(
			join(tmpdir(), "performance-control-migration-"),
		);
		...
		try {
			...
			const projection = execFileSync(
				...
				[
					database,
					"SELECT control_state, data_mode, COALESCE(snapshot_key, ''), COALESCE(snapshot_sha256, ''), legacy_snapshot_sha256 FROM project_state WHERE project_slug='performance-observability-control';",
				],
				{ encoding: "utf8" },
			).trim();
	...
	it("enables controls only through an explicit production-complete command", () => {
		expect(() =>
			execFileSync(
				"node",
				[
					resolve(root, "scripts/generate-production-config.mjs"),
					"--enable-control",
				],
		...
		expect(
			(JSON.parse(output) as { vars: { CONTROL_ENABLED: string } }).vars
				.CONTROL_ENABLED,
		).toBe("true");
	...
	it("generates a deterministic production config that remains disabled", () => {
		const output = execFileSync(
			"node",
			[resolve(root, "scripts/generate-production-config.mjs"), "--ci"],
			{ encoding: "utf8" },
		);
		...
		expect(config.r2_buckets).toEqual([
			expect.objectContaining({
				bucket_name: "baby2b-performance-snapshots-ci",
			}),
		]);
	...
	it("keeps the remote gate on the canonical Baby2B D1 name", () => {
		const workflow = readFileSync(
			resolve(
				root,
				"../../../../.github/workflows/verify-performance-control.yml",
			),
			"utf8",
		);
