...
export const PORTFOLIO_PROJECTS: PortfolioProject[] = [
	{
		...
		desc: "AI Agent 分布式任务调度 / Web3 Agent 平台。参与 Job 编辑发布、语义分类与任务拆解、Agent 匹配派发、异步反馈、结果聚合、合约托管结算及 AI 内容生产链路。",
		summaryPoints: [
			"任务编排：参与 Job 编辑与发布流程，覆盖分类、标签、预算、时限、技能和交付物，并支持自动拆解与人工编排。",
			"调度链路：接入 Agent 匹配、派发、生命周期、异步反馈和结果聚合，以请求 ID、Webhook / SSE、消息队列和并发控制保证可追踪与可恢复。",
			"Web3 与内容链路：联调 MetaMask、合约托管 / 结算、IPFS / 链上证据与 AI 内容生产流程，协同 PostgreSQL / pgvector、GraphQL 和 AWS 服务。",
		],
		status: "已完成",
		progress: 100,
		architecture:
		...
		details: [
			"参与 Job 表单与发布链路，整理分类、标签、预算、时限、技能和交付物，并支持自动拆解后的人工调整。",
			"参与 Agent 匹配派发、生命周期、异步反馈和结果聚合，通过请求 ID、Webhook / SSE、消息队列与并发控制串联任务状态。",
			"参与 MetaMask 签名、智能合约托管 / 结算、IPFS / 链上结果证据及端到端联调。",
			"参与 AI 内容工作流，将创意逐步推进为剧本、角色、场景、分镜、视频片段及最终合成。",
	...
	},
	{
		...
		progress: 100,
		architecture:
			"Edge SSR + React Hydration + GraphQL / LangGraph DAG + Node / Hono、Python 与 Go 多运行时服务 + PostgreSQL Checkpoint + 异步队列 / DLQ + 信誉评分与 LLM 评测；以结构化任务状态支撑暂停恢复、人工介入和复杂任务分发。",
	...
	},
	{
		...
		progress: 100,
		architecture:
			"已交付训练链路：LlamaFactory + Qwen3-8B + NF4 QLoRA → Adapter 合并 → GGUF 量化 → Ollama；客服系统方案：意图路由 → Qwen Embedding 检索与知识图谱 / RAG → Tool Calling 与结构化业务规则 → 低置信度 / 敏感问题转人工。",
