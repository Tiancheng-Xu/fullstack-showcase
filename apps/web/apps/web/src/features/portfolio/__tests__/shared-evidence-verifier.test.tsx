...
describe("Shared Evidence Verifier portfolio delivery", () => {
	it("publishes the verified cloud project on the Dashboard", () => {
		...
		expect(
			screen.getAllByRole("heading", { name: "Shared Evidence Verifier" }),
		).not.toHaveLength(0);
	...
	it("shows architecture, sequence, proof boundaries and fullscreen preview", () => {
		...
		expect(
			screen.getByRole("heading", { name: "共享验证架构图" }),
		).toBeVisible();
		expect(
			screen.getByRole("heading", { name: "串行验证时序图" }),
		).toBeVisible();
		expect(
			screen.getByRole("heading", { name: "交付要求到证据映射" }),
		).toBeVisible();
		expect(screen.getAllByText(/verified-with-limitations/)).not.toHaveLength(
			0,
		);
		...
		expect(
			screen.getByRole("dialog", { name: "共享验证架构图" }),
		).toBeVisible();
		fireEvent.click(screen.getByRole("button", { name: "关闭全屏预览" }));
		expect(
			screen.queryByRole("dialog", { name: "共享验证架构图" }),
