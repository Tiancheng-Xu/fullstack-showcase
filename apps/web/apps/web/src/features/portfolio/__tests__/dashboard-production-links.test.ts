...
describe("portfolio production contracts", () => {
	it("keeps static-first links identical before and after remote hydration", () => {
		...
		expect(agentMarket?.evidence.join(" ")).toContain("3058bb66cff3");
		expect(agentMarket?.evidence.join(" ")).toContain("33246537531");
		expect(agentMarket?.evidence.join(" ")).toContain("6 项 IMPLEMENTED_UNVERIFIED 已升级 VERIFIED");
		expect(agentMarket?.details.join(" ")).toContain("2bd19543");
		expect(agentMarket?.details.join(" ")).toContain("source 3058bb6");
