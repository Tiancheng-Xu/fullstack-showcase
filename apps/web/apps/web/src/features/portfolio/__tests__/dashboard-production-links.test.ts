...
describe("portfolio production contracts", () => {
	it("keeps static-first links identical before and after remote hydration", () => {
		...
		expect(agentMarket?.evidence.join(" ")).toContain("4589c72b110f");
		expect(agentMarket?.evidence.join(" ")).toContain("34088481986");
		expect(agentMarket?.evidence.join(" ")).toContain("34088482435");
		expect(agentMarket?.details.join(" ")).toContain("15f7837e");
		expect(agentMarket?.details.join(" ")).toContain("source 4589c72b110f");
